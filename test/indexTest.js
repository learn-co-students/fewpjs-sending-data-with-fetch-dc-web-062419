...
describe('submitData()', () => {
  ...
  it('makes a POST request to /user with a name and email', async () => {
    ...
    expect(
      Object.keys(reqBody),
      "The request body should only have 'name' and 'email' key/value pairs",
    ).to.deep.equal(['name', 'email']);
    expect(
      reqBody.name,
      "The 'name' property was not found in the request body",
    ).to.eq('Steve');
    expect(
      reqBody.email,
      "The 'email' property was not found in the request body",
